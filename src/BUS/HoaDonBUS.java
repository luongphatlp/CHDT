...
public class HoaDonBUS {
    ...
    public ArrayList<HoaDonDTO> timKiem(String key, String pttt, String nv,
        ...
        if (tungay == null) {
            tungay = LocalDate.MIN;
        }
        if (denngay == null) {
            denngay = LocalDate.MAX;
        }
    ...
    public void xuatPDF(String path, String mahd) {
        ...
        if (hd == null) {
            System.out.print("Lỗi không tìm thấy hóa đơn");
            return;
        }
    ...
    public void xuatPDF(String path, String mahd, String manv, String makh, String ngay, String tongtien, String pttt) {
        try {
            ...
            Font fontHeader = new Font(bf, 18, Font.BOLD);

            Document document = new Document();
            PdfWriter.getInstance(document, new FileOutputStream(path));
            document.open();

            // 2. Tiêu đề cửa hàng và Hóa đơn
            Paragraph shopName = new Paragraph("CỬA HÀNG ĐIỆN THOẠI TNFTN", fontBold);
            document.add(shopName);
            document.add(new Paragraph("Địa chỉ: 273 An Dương Vương, Phường Chợ Quán, TP. HCM", fontNormal));
            document.add(new Paragraph("Hotline: 0913.686.868", fontNormal));

            Paragraph title = new Paragraph("\nHÓA ĐƠN BÁN HÀNG", fontHeader);
            title.setAlignment(Element.ALIGN_CENTER);
            document.add(title);
            document.add(new Paragraph(" ")); // Dòng trống

            // 3. Thông tin chung (Sử dụng bảng không viền để căn chỉnh đẹp hơn)
            PdfPTable infoTable = new PdfPTable(2);
            infoTable.setWidthPercentage(100);
            infoTable.setWidths(new float[]{1.5f, 1f}); // Cột trái rộng hơn

            // Hàm phụ để thêm cell không viền
            addInfoCell(infoTable, "Mã hóa đơn: " + mahd, fontNormal, Element.ALIGN_LEFT);
            addInfoCell(infoTable, "Ngày lập: " + ngay, fontNormal, Element.ALIGN_RIGHT);
            addInfoCell(infoTable, "Nhân viên: " + manv, fontNormal, Element.ALIGN_LEFT);
            addInfoCell(infoTable, "Phương thức: " + pttt, fontNormal, Element.ALIGN_RIGHT);
            addInfoCell(infoTable, "Khách hàng: " + makh, fontNormal, Element.ALIGN_LEFT);
            addInfoCell(infoTable, "", fontNormal, Element.ALIGN_RIGHT);

            document.add(infoTable);
            document.add(new Paragraph(" "));

            // 4. Bảng danh sách sản phẩm (6 cột)
            ...
            // Đổ dữ liệu từ ArrayList
            ArrayList<ChiTietHoaDonDTO> tam = getCTHDByMaHD(mahd);
            int i = 1;
            for (ChiTietHoaDonDTO ct : tam) {
                table.addCell(createCell(String.valueOf(i++), fontNormal, Element.ALIGN_CENTER));
                table.addCell(createCell(ct.getMaSP(), fontNormal, Element.ALIGN_CENTER));
                table.addCell(createCell(ct.getTenSP(), fontNormal, Element.ALIGN_LEFT));
                table.addCell(createCell(String.format("%,.0f", (double) ct.getDonGia()), fontNormal, Element.ALIGN_RIGHT));
                table.addCell(createCell(String.valueOf(ct.getSoLuong()), fontNormal, Element.ALIGN_CENTER));
                table.addCell(createCell(String.format("%,.0f", (double) ct.getThanhTien()), fontNormal, Element.ALIGN_RIGHT));
            }

            document.add(table);

            // 5. Tổng tiền
            Paragraph total = new Paragraph("\nTỔNG TIỀN THANH TOÁN: " + tongtien + " VNĐ", fontBold);
            ...
            addInfoCell(signTable, "\nNgười mua hàng\n(Ký và ghi rõ họ tên)", fontNormal, Element.ALIGN_CENTER);
            addInfoCell(signTable, "Ngày....tháng....năm....\nNgười bán hàng\n(Ký và ghi rõ họ tên)", fontNormal, Element.ALIGN_CENTER);
...
// Hàm bổ trợ để tạo Cell không viền cho phần thông tin
